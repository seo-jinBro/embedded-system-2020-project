...
class QuantizedBertEmbeddings(BertEmbeddings):
    def __init__(self, config):
        ...
        self.token_type_embeddings = quantized_embedding_setup(
            config, config.type_vocab_size, config.hidden_size
        )
...
class QuantizedBertPreTrainedModel(BertPreTrainedModel):
    ...
    def init_weights(self, module):
        ...
        if isinstance(
            module, (nn.Linear, nn.Embedding, QuantizedLinear, QuantizedEmbedding)
        ):
    ...
    @classmethod
    def from_pretrained(
        cls, pretrained_model_name_or_path, *args, from_8bit=False, **kwargs
    ):
...
class QuantizedBertForSequenceClassification(
    ...
    def __init__(self, config):
        ...
        self.classifier = quantized_linear_setup(
            config, config.hidden_size, self.config.num_labels
        )

        self.apply(self.init_weights)


class QuantizedBertForQuestionAnswering(
    QuantizedBertPreTrainedModel, BertForQuestionAnswering
):
    def __init__(self, config):
        # we only want BertForQuestionAnswering init to run to avoid unnecessary
        # initializations
...
class QuantizedBertForTokenClassification(
    QuantizedBertPreTrainedModel, BertForTokenClassification
):
    def __init__(self, config):
